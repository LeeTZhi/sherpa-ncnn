#!/usr/bin/env bash
export PATH=/home/ltz/nvme/Audio_code/toolchain/arm-sgmstar-gnueabihf-9.1.0-202007/gcc-sigmastar-9.1.0-2020.07-x86_64_arm-linux-gnueabihf/bin:$PATH

if ! command -v arm-linux-gnueabihf-sgmstar-202007-gcc "-v" &> /dev/null; then
...
dir=build-aarch64-linux-gnu-sgmstar
mkdir -p $dir
cd $dir
...
if [ ! -f alsa-lib/src/.libs/libasound.so ]; then
  ...
  #
  pushd alsa-lib
  CC=arm-linux-gnueabihf-sgmstar-202007-gcc ./gitcompile --host=aarch64-linux-gnu
  popd
...
cmake \
  ...
  -DCMAKE_TOOLCHAIN_FILE=../toolchains/aarch64-linux-gnu-sgmstar.cmake \
  ..
