#!/usr/bin/env bash
export PATH=$PATH:/home/ltz/nvme/Audio_code/toolchain/toochain-gcc-3568/host/bin

if ! command -v aarch64-linux-gcc "-v" &> /dev/null; then
...
dir=build-aarch64-linux-gnu-3568
mkdir -p $dir
cd $dir
...
if [ ! -f alsa-lib/src/.libs/libasound.so ]; then
  ...
  #
  pushd alsa-lib
  CC=aarch64-linux-gcc ./gitcompile --host=aarch64-linux-gnu
  popd
...
cmake \
  ...
  -DCMAKE_TOOLCHAIN_FILE=../toolchains/aarch64-linux-gnu-3568.toolchain.cmake \
  ..
